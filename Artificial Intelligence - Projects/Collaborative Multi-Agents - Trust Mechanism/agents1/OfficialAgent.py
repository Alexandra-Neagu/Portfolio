...
class BaselineAgent(ArtificialBrain):
    def __init__(self, slowdown, condition, name, folder, eval_type):
        ...
        self.trust = Trust(name, folder, eval_type) # Agent's trust model for the human
    ...
    def decide_on_actions(self, state):
        ...
        for mssg in self.received_messages:
            for member in self._teamMembers:
                if mssg.from_id == member and (mssg.content + ' ' + mssg.message_id) not in self._receivedMessages:
                    self._receivedMessages.append(mssg.content + ' ' + mssg.message_id)
        ...
        # NOTE: Refactored to agent's __init__ method.
        # We have to change this a whole bunch, so I refactored it to its own class
        # Also please use pythonic naming conventions for consistency in the stuff we do. 
        # They're using several kinds in this class, but not worth touching those.

        # self._trustBelief(trustBeliefs)
        # NOTE: Update trust with messages, as was before.
        # NOTE: it's stupid: It reads the same messages every game tick.
        # For now, I made the trust model maintain a list of 'read' messages,
        # But this can become laggy towards the end of the game. 
        self.trust.update_belief(messages=self._receivedMessages)
        ...
        while True:
            ...
            if Phase.FIND_NEXT_GOAL == self._phase:
                ...
                # This should be the only point in the code where the agent can be idle
                if not remainingZones:
                    return None, {}

                # Check which victims can be rescued next because human or agent already found them             
                # TODO: @Doga: This is probably where you would compare the human and robot's rescue lists
                # Alternatively though, I'm lowering the human's willingness if they have lied about
                # collecting victims, see PICK_UNSEARCHED_ROOM > # If all areas have been searched...
            ...
            if Phase.PICK_UNSEARCHED_ROOM == self._phase:
                ...
                if self._remainingZones and len(unsearchedRooms) == 0:
                    ...
                    # This would mean the human lied. Lower willingness.
                    # Even though this would not affect much this game round, it will be useful
                    # for future game rounds.
                    self.trust.decrease_willingness('All areas have been searched but task is not finished. Human '
                                                    'lied about the areas they searched.')
                ...
                for info in state.values():
                    ...
                    if 'class_inheritance' in info and 'ObstacleObject' in info['class_inheritance'] and 'rock' in info['obj_id']:
                        ...
                        if self.received_messages_content and self.received_messages_content[-1] == 'Continue' and not self._remove:
                            ...
                            # NOTE: Big rock can only be removed together, so there is nothing we can do.
                            # Could consider repeatedly prompting the human, but eventually the game will
                            # do this anyway.

                            # NOTE: Because received a message in time.
                            self.trust.reset_timeout()
                            self.trust.increase_willingness('for replying in time to continue on removing a rock.')
                        ...
                        # Reset waiting timeout on message received
                        if self.received_messages_content and self.received_messages_content[-1] == 'Remove':
                            self.trust.reset_timeout()

                        # Wait for the human to come and help remove the obstacle together
                        if self.received_messages_content and self.received_messages_content[-1] == 'Remove' or self._remove:
                            ...
                            if not state[{'is_human_agent': True}]:
                                ...
                                # Robot remains idle until human arrives to remove the obstacle
                                # Introduce an idling timeout to prevent robot from remaining idle indefinitely
                                # If the human does not arrive, reduce willingness
                                if self.trust.waiting(self, 'remove', f'human show up for removing a big rock'):
                                    return None, {}
                                else:
                                    self._sendMessage(f'Moving on to the next goal and not removing rock blocking {str(self._door["room_name"])}, because you did not show up... ','RescueBot')
                                    # Add area to the to do list
                                    self._waiting = False
                                    self._tosearch.append(self._door['room_name'])
                                    self._phase = Phase.FIND_NEXT_GOAL
                            ...
                            if state[{'is_human_agent': True}]:
                                ...
                                # Robot remains idle until human removes the obstacle
                                # Introduce an idling timeout to prevent robot from remaining idle indefinitely
                                # If the human does not remove the obstacle, reduce willingness

                                # TODO: There seems to be no provided way to check whether the human is actually
                                # removing the big rock. This means that if the waiting timer runs out during this operation,
                                # the robot will simply move on, even though the rock is also removed.

                                if self.trust.waiting(self, 'remove', f'removal of a big rock'):
                                    return None, {}
                                else:
                                    self._sendMessage(f'Moving on to the next goal and not removing rock blocking {str(self._door["room_name"])}, because you did not actually remove the rock... ','RescueBot')
                                    # Add area to the to do list
                                    self._waiting = False
                                    self._remove = False
                                    self._tosearch.append(self._door['room_name'])
                        ...
                        # Remain idle until the human communicates what to do with the identified obstacle
                            ...
                            # Robot remains idle indefinitely until human decides what to do with the obstacle
                            # Introduce an idling timeout to prevent robot from remaining idle indefinitely
                            # If the human does not decide what to do, reduce willingness
                            if self.trust.waiting(self, 'remove', f'on response on removing a big rock'):
                                return None, {}
                            
                            else:
                                self._sendMessage(f'Moving on to the next goal and not removing rock blocking {str(self._door["room_name"])}, because you did not reply... ','RescueBot')
                                # Add area to the to do list
                                self._waiting = False
                                self._tosearch.append(self._door['room_name'])
                                self._phase = Phase.FIND_NEXT_GOAL
                    ...
                    if 'class_inheritance' in info and 'ObstacleObject' in info['class_inheritance'] and 'tree' in info['obj_id']:
                        ...
                        # Determine the next area to explore if the human tells the agent not to remove the obstacle
                        if self.received_messages_content and self.received_messages_content[-1] == 'Continue' and not self._remove\
                            and self.trust.willing('not removing a tree'):
                            ...
                            # NOTE: Removing a tree is almost always beneficial, it is an independent action.
                            # Unless willingness is very high, robot should remove the tree.
                            # Optionally can reduce willingness here, but would result in fluctuating values.
                            self.trust.decrease_willingness(f'you asked me to skip removing a tree (independent action that can only be performed by robot)')
                        ...
                        if self.received_messages_content and self.received_messages_content[-1] == 'Remove' or self._remove:
                            ...
                            if not self._remove:
                                ...
                                # NOTE: Human approved of this action within the timeout.
                                self._sendMessage('Removing tree blocking ' + str(self._door['room_name']) + '.','RescueBot')
                                self.trust.increase_willingness('you replied within time. this is productive')

                            if self._remove:

                                # NOTE: The human called the robot over for help in removing an obstacle
                                self._sendMessage('Removing tree blocking ' + str(self._door['room_name']) + ' because you asked me to.', 'RescueBot')
                                self.trust.increase_willingness('you asked me to remove an object only I can remove')
                        ...
                        # Remain idle until the human communicates what to do with the identified obstacle
                            ...
                            # NOTE: The robot is waiting for a response from the human
                            if self.trust.waiting(self, 'remove', f'response on removing a tree'):
                                return None, {}
                                ...
                                self._waiting = False
                                self._answered = True
                                self._sendMessage(f'Removing tree blocking {str(self._door["room_name"])}, because you did not reply...', 'RescueBot')
                                return RemoveObject.__name__, {'object_id': info['obj_id']}
                    ...
                    if 'class_inheritance' in info and 'ObstacleObject' in info['class_inheritance'] and 'stone' in info['obj_id']:
                        ...
                        if self.received_messages_content and self.received_messages_content[-1] == 'Continue' and not self._remove:
                            ...
                            # If willingness is low, robot should remove the rock (default to 'Remove alone').
                            # Human answered within timeout, so don't change willingness.
                            if not self.trust.willing():
                        ...
                        if self.received_messages_content and self.received_messages_content[-1] == 'Remove alone' and not self._remove:
                            ...
                            # Human answered within timeout, so increase willingness.
                            self.trust.increase_willingness('you replied within time. this is productive')
                        ...
                        if self.received_messages_content and self.received_messages_content[-1] == 'Remove together' or self._remove:
                            ...
                            # Human could be lying about coming to help.
                            # If willingness is low, robot should remove the small rock alone.
                            if not self.trust.willing():
                                self._sendMessage('Removing stones blocking ' + str(self._door['room_name']) + '.',
                                                  'RescueBot')
                                self._phase = Phase.ENTER_ROOM
                                self._remove = False
                                self.trust.decrease_competence('you asked for help to remove a small rock together.')
                            ...
                            if not state[{'is_human_agent': True}]:
                                ...
                                # Introduce an idling timeout to prevent robot from remaining idle indefinitely
                                # If the human does not arrive, reduce willingness
                                if self.trust.waiting(self, 'remove', f'human show up for removing a small rock'):
                                    return None, {}
                                # If the human does not show up, reduce willingness and default to 'Remove alone'.
                                else:
                                    self._sendMessage('Removing stones blocking ' + str(self._door['room_name']) + '.',
                                                      'RescueBot')
                                    self._phase = Phase.ENTER_ROOM
                            ...
                            if state[{'is_human_agent': True}]:
                                ...
                                # Introduce an idling timeout to prevent robot from remaining idle indefinitely
                                # If the human does not remove the obstacle, reduce willingness
                                if self.trust.waiting(self, 'remove', f'on response on removing a small rock'):
                                    return None, {}

                                # If the human does not remove, reduce willingness and default to 'Remove alone'.
                                else:
                                    self._sendMessage('Removing stones blocking ' + str(self._door['room_name']) + '.',
                                                      'RescueBot')
                                    self._phase = Phase.ENTER_ROOM
            ...
            if Phase.FOLLOW_ROOM_SEARCH_PATH == self._phase:
                        ...
                        if 'class_inheritance' in info and 'CollectableBlock' in info['class_inheritance']:
                            ...
                            if vic in self._foundVictims and 'location' not in self._foundVictimLocs[vic].keys():
                                ...
                                if vic == self._goalVic:
                                    ...
                                    # Victim found as human told: Increase willingness
                                    self.trust.increase_willingness(msg='victim is found as human told')
                ...
                if self._goalVic in self._foundVictims and self._goalVic not in self._roomVics and self._foundVictimLocs[self._goalVic]['room'] == self._door['room_name']:
                    ...
                    # Human lied about the location of the victim, or rescued the victim without telling the agent
                    # Reduce willingness
                    self.trust.decrease_willingness(msg= 'human lied about the location of the victim')
                ...
                if self.received_messages_content and self.received_messages_content[-1] == 'Rescue' and 'critical' in self._recentVic:
                    self.trust.reset_timeout()
                    # Human replied in time for rescuing the victim alone, increase willingness
                    self.trust.increase_willingness('You replied within time. This is productive')
                    self._rescue = 'together'
                    self._answered = True
                    self._waiting = False
                ...
                if self.received_messages_content and self.received_messages_content[-1] == 'Rescue together' and 'mild' in self._recentVic:
                    ...
                    # Human replied in time for rescuing the victim alone, increase willingness
                    self.trust.increase_willingness('You replied within time. This is productive')
                    # If a human asks for help to rescue a mildly injured victim, they could be weak. Thus, reduce competence
                    self.trust.decrease_competence('You asked for help to rescue a mildly injured victim')

                    #If willingness is low, default to 'Rescue alone'. This is because the lying human may not show up.
                    #Otherwise, have an idling timeout. If the human does not show up, reduce willingness and default to rescue alone.
                    if not self.trust.willing():
                ...
                if self.received_messages_content and self.received_messages_content[-1] == 'Rescue alone' and 'mild' in self._recentVic:
                    ...
                    # Human replied in time for rescuing the victim alone
                ...
                if self.received_messages_content and self.received_messages_content[-1] == 'Continue':
                    ...
                    #Human asks robot to continue searching, even though this may not be optimal
                    self.trust.reset_timeout()
                    # If human is not willing, and the victim is mildly injured, rescue alone
                    if not self.trust.willing() and self._recentVic and 'mild' in self._recentVic:
                        self._sendMessage('Picking up ' + self._recentVic + ' in ' + self._door['room_name'] + '.','RescueBot')
                        self._rescue = 'alone'
                        self._answered = True
                        self._waiting = False
                ...
                if self.received_messages_content and self._waiting and self.received_messages_content[-1] != 'Rescue' and self.received_messages_content[-1] != 'Continue':
                    ...
                    # TODO: Robot may remain idle for too long if the human does not communicate in time
                    # Reduce willingness. If the victim is critical, move on to the next task, otherwise, rescue alone.
                    if self.trust.waiting(self, 'rescue', f'response on rescuing a victim'):
                        return None, {}
                    else:
                        # if the victim is mildly injured, rescue alone
                        if self._recentVic and 'mild' in self._recentVic:
                            self._sendMessage('Picking up ' + self._recentVic + ' in ' + self._door['room_name'] + '.','RescueBot')
                            self._rescue = 'alone'
                        else:
                            # if the victim is critical, continue
                            self._todo.append(self._recentVic)
                        self._answered = True
                        self._waiting = False
                        self._recentVic = None
            ...
            if Phase.FOLLOW_PATH_TO_VICTIM == self._phase:
                ...
                if self._goalVic and self._goalVic in self._collectedVictims:
                    ...
                    # Human rescued the victim by themselves
                    # Increase competence.
                    self.trust.increase_competence('You rescued the victim by yourself!')
            ...
            if Phase.TAKE_VICTIM == self._phase:
                ...
                for info in state.values():
                    ...
                    if 'class_inheritance' in info and 'CollectableBlock' in info['class_inheritance'] and 'critical' in info['obj_id'] and info['location'] in self._roomtiles or \
                        ...
                        if not self._humanName in info['name']:
                            ...
                            # TODO: Forever waiting for human to arrive at victim location
                            # Reduce willingness after timeout.
                            # if trust is low, just continue searching
                ...
                if len(objects) == 0 and 'critical' in self._goalVic or len(objects) == 0 and 'mild' in self._goalVic and self._rescue=='together':
                    ...
                    #Human and robot have rescued the victim together, increase willingnes.
                    self.trust.increase_willingness('we have rescued a victim together.')
    ...
    def _loadBelief(self, members, folder):
        '''
        OLD FUNCTION. I left it in case they want to use it for something.
        But good code style includes not having everything in one damn file.
        ...
        '''
        return trust.load_belief(self._humanName, folder)

    def _trustBelief(self, trust_beliefs):
        '''
        Baseline implementation of a trust belief. 
        Returns a dictionary with trust belief scores for each team member, for example based on the received messages.
        '''
        return trust.update_belief(self, trust_beliefs)
