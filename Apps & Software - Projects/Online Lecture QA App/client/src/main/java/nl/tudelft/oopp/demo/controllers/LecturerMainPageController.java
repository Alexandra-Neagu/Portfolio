...
/**
 * The type Lecturer main page controller.
 */
public class LecturerMainPageController implements Initializable {
    ...
    /**
     * Too fast too slow text..
     */
    @FXML
    Label tooFastTooSlow;
    ...
    /**
     * Checkbox for switching to the simplified view.
     */
    @FXML
    CheckBox checkBox;
     ...
     */
    public void closeRoom(ActionEvent actionEvent) throws IOException {
        ...
        if (alert.getResult() == ButtonType.YES) {
            MainApp.setCurrentRoom(RoomCommunication.closeRoom());
            ...
            try {
                if (checkBox.isSelected()) {
                    FXMLLoader loader = new FXMLLoader(getClass()
                            .getResource("/listItemLecturer.fxml"));
                    node = (Node) loader.load();
                    ListItemLecturerController listItemLecturerController = loader.getController();
                    listItemLecturerController.fillQuestion(q);
                    questionHolder.getChildren().add(node);
                } else {
                    FXMLLoader loader = new FXMLLoader(getClass()
                            .getResource("/listItemLecturerComplex.fxml"));
                    node = (Node) loader.load();
                    ListItemLecturerComplexController
                            listItemLecturerComplexController = loader.getController();
                    listItemLecturerComplexController.fillQuestion(q);
                    questionHolder.getChildren().add(node);
                }
    ...
    /**
     * Checks if the lecture is going too fast or too slow.
     */
    public void tooFastTooSlow() {
        int studentCount = RoomCommunication.getStudentCount();
        int tooFastCount = RoomCommunication.getTooFastCount();
        int tooSlowCount = RoomCommunication.getTooSlowCount();

        double percentStudentCount = studentCount * 0.2;

        tooFastTooSlow.setText("No issue with pace.");

        if (percentStudentCount < tooSlowCount && tooFastCount < tooSlowCount
                && tooSlowCount >= 5) {
            tooFastTooSlow.setText("Pace is too slow!");
        } else if (percentStudentCount < tooFastCount
                && tooSlowCount < tooFastCount && tooFastCount >= 5) {
            tooFastTooSlow.setText("Pace is too fast!");
        } else if (Objects
                .equals(tooFastCount, tooSlowCount) && tooFastCount < 5) {
            tooFastTooSlow.setText("No issue with pace.");
        } else if (Objects
                .equals(tooFastCount, tooSlowCount) && percentStudentCount < tooSlowCount
                && tooFastCount >= 5) {
            tooFastTooSlow.setText("Students are indecisive.");
        }
    }

    /**
     * Reset the vote count for the pace of the lecture.
     *
     * @param event the event
     */
    public void resetButton(ActionEvent event) {
        RoomCommunication.setTooFastCount();
        RoomCommunication.setTooSlowCount();
        EventLog.logResetPace(MainApp.getCurrentUser(), MainApp.getCurrentRoom());
    }
    ...
    /**
     * When the create poll button is clicked
     * this method will make sure the screen
     * transition happens.
     *
    ...
    /**
     * Checks, collects and updates the amount of votes
    ...
    /**
     * This method makes sure ths poll stats
     * questions that are displayed are correct
     * and actually being displayed.
     */
    ...
    /**
     * Makes it so the buttons are updated and all text is shown
     * correctly.
     *
     * @param number the amount of options in the poll
     * @param poll   the current poll that has been opened
     */
